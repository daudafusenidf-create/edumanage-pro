window._EDUMANAGE_VERSION = 'v2026.AUTH.1';
/* ════════════════════════════════════════
...
function saveFee() {
  ...
  var fDueEl = document.getElementById('fDue');
  var manualAmt = fDueEl ? parseFloat(fDueEl.value) || 0 : 0;
  var due = (structAmt !== null && structAmt > 0) ? structAmt : manualAmt;
  ...
  document.getElementById('photoUploadInput').addEventListener('change', function(e) {
    ...
    files.forEach(function(file) {
      ...
      reader.onload = function(ev) {
        ...
        loaded++;
        if (loaded === files.length) { renderAlbumPhotos(album); saveToDB(); showToast(loaded + ' photo(s) added!'); }
      };
      reader.readAsDataURL(file);
    });
  });
...
function initLogo() {
  ...
  fileInput.addEventListener('change', function(e) {
    ...
    var reader = new FileReader();
    reader.onload = function(ev) { state.schoolLogo = ev.target.result; applyLogo(ev.target.result); saveToDB(); showToast('School logo uploaded!'); };
    reader.readAsDataURL(file);
  });
...
function attemptLogin() {
  ...
  // Lockout check (applies to both auth paths)
  ...
  if (isEmail && window._fbAuth && _isOnline) {
    // ── PATH A: Firebase Auth (for admins with email) ──
    window._fbAuth.signIn(input, password)
      .then(function(userCredential) {
        // Firebase Auth succeeded — find matching user in school data
        const email = input.toLowerCase();
        const schoolKey = getSchoolKey(schoolId);

        const findAndProceed = function() {
          // Look for user with matching email in state.users
          let user = (state.users || []).find(function(u) {
            return u.email && u.email.toLowerCase() === email && u.active !== false;
          });
          if (!user) {
            // Firebase Auth passed but no matching user record — could be first login
            // or email not yet linked. Show clear error.
            window._fbAuth.signOut();
            document.getElementById('loginErrorMsg').textContent =
              'Firebase Auth succeeded but no user record found for ' + email +
              '. Ask your Super Admin to link this email to a user account.';
            document.getElementById('loginError').style.display = 'block';
            resetBtn(); return;
          }
          resetLoginAttempts(schoolId);
          user._authMethod = 'firebase';
          proceedWithUser(user, schoolId, schoolKey, input, password);
        };

        // Load school data first if not loaded
        if (!state.currentUser && schoolId) {
          const schoolKey = getSchoolKey(schoolId);
          if (window._fbReady && _isOnline) {
            loadSchoolData(schoolKey);
            loadSchoolDataFromFirebase(schoolId)
              .then(findAndProceed)
              .catch(findAndProceed);
          } else {
            loadSchoolData(schoolKey);
            findAndProceed();
          }
      ...
      .catch(function(err) {
        ...
        const tries = (_loginAttempts[schoolId] && _loginAttempts[schoolId].count) || 0;
        const left = LOGIN_MAX_TRIES - tries;
        let msg = 'Incorrect email or password.';
        if (err.code === 'auth/user-not-found' || err.code === 'auth/wrong-password' || err.code === 'auth/invalid-credential') {
          msg = 'Incorrect email or password. ' + left + ' attempt' + (left !== 1 ? 's' : '') + ' left.';
    ...
    // ── PATH B: Username/password (for teachers and students) ──
    const username = input.toLowerCase();
    const schoolKey = getSchoolKey(schoolId);

    const tryLogin = function() {
      let user = (state.users || []).find(function(u) {
        return u.username && u.username.toLowerCase().trim() === username &&
               u.password && u.password.trim() === password &&
               u.active !== false;
      });
      // Fallback: try localStorage if state is stale
      if (!user) {
        try {
          const localRaw = localStorage.getItem(schoolKey);
          if (localRaw) {
            const localData = JSON.parse(localRaw);
            if (localData.users) {
              user = localData.users.find(function(u) {
                return u.username && u.username.toLowerCase().trim() === username &&
                       u.password && u.password.trim() === password &&
                       u.active !== false;
              });
              if (user) state.users = localData.users;
            }
          }
        } catch(e) {}
      }
      ...
      resetLoginAttempts(schoolId);
      user._authMethod = 'local';
      proceedWithUser(user, schoolId, schoolKey, username, password);
    };

    if (window._fbReady && _isOnline) {
      loadSchoolData(schoolKey);
      let done = false;
      const timer = setTimeout(function() { if (!done) { done = true; tryLogin(); } }, 6000);
      loadSchoolDataFromFirebase(schoolId)
        .then(function() { if (!done) { done = true; clearTimeout(timer); tryLogin(); } })
        .catch(function() { if (!done) { done = true; clearTimeout(timer); tryLogin(); } });
    } else {
      loadSchoolData(schoolKey);
      tryLogin();
    }
...
// ── Shared post-login logic for both auth paths ──
function proceedWithUser(user, schoolId, schoolKey, identifier, password) {
  const loginBtn = document.getElementById('loginBtn');
  ...
  if (rememberMe && rememberMe.checked) {
    localStorage.setItem('edumanage_remembered_' + schoolId, JSON.stringify({ username: identifier, password }));
  } else {
...
function doLogout() {
  saveNow();
  stopRealtimeSync();
  // Sign out of Firebase Auth if admin was using it
...
function saveNewPassword() {
  var current = document.getElementById('cpCurrentPwd').value;
  var newPwd  = document.getElementById('cpNewPwd').value.trim();
  var confirm = document.getElementById('cpConfirmPwd').value.trim();
  ...
  var isFirebaseAdmin = state.currentUser && state.currentUser._authMethod === 'firebase';
  if (isFirebaseAdmin && window._fbAuth) {
    // Admin: change via Firebase Auth (secure)
    window._fbAuth.updatePassword(current, newPwd)
      .then(function() {
    ...
    // Teacher/Student: change in local state
...
function forgotPassword() {
  var input = document.getElementById('loginUsername').value.trim();
  var isEmail = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(input);
  if (!isEmail) {
    showToast('Enter your email address in the login field first, then click Forgot Password.');
    return;
  }
  if (!window._fbAuth || !_isOnline) {
    showToast('Internet required to reset password.');
    return;
  }
  window._fbAuth.sendPasswordReset(input)
    .then(function() {
      showToast('Password reset email sent to ' + input + '. Check your inbox.');
    })
    .catch(function(err) {
      if (err.code === 'auth/user-not-found') {
        showToast('No account found for ' + input + '. Contact your Super Admin.');
      } else {
...
function saveUser() {
  ...
  // Email required for Admin (Firebase Auth)
  var emailEl = document.getElementById('uEmail');
  var email = emailEl ? emailEl.value.trim().toLowerCase() : '';
  if (role === 'Admin' && !email) {
    showToast('Email is required for Admin accounts (used for Firebase Auth login).'); return;
  }
  ...
  if (editId) {
    ...
    state.users.push({ id:state.nextUserId++, name, username, password, email: email||'', role, linkedStudentId, permissions, active:true });
    // If Admin with email, create Firebase Auth account
    if (role === 'Admin' && email && window._fbAuth && _isOnline) {
      window._fbAuth.createUser(email, password)
        .then(function() { showToast('Admin account created with Firebase Auth login.'); })
        .catch(function(err) {
          if (err.code !== 'auth/email-already-in-use') {
            console.warn('[Auth] createUser failed:', err.message);
          }
        });
    }
...
function printPortalReport(reportId) {
  ...
  var rows = (r.subjects||[]).map(function(sub) {
    return "<tr><td>"+sub.name+"</td><td>"+(sub.classScore||"-")+"</td><td>"+(sub.examScore||"-")+"</td><td style=\"font-weight:700;color:"+((sub.total||0)>=50?"#16a34a":"#dc2626")+"\">"+(sub.total||"-")+"</td><td>"+(sub.grade||"-")+"</td></tr>";
  }).join("");
  win.document.write("<!DOCTYPE html><html><head><title>Report Card</title><style>body{font-family:Arial,sans-serif;padding:20px;max-width:700px;margin:auto;}table{width:100%;border-collapse:collapse;margin-top:12px;}th,td{border:1px solid #ddd;padding:8px;}th{background:#1a6fd4;color:#fff;}</style></head><body>"
    ...
    +"<table><tr><th>Subject</th><th>Class Score</th><th>Exam Score</th><th>Total</th><th>Grade</th></tr>"+rows+"</table>"
