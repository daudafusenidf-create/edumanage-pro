...
function _applyDataToState(data) {
  ...
  merge('classSubjects',[]);  // ✅ NEW: subject assignments per class
...
async function submitSchoolRegistration(name, adminName, adminUser, adminPass, phone, email) {
  ...
  const freshData = {
    students:[], fees:[], feeStructure:[], classBills:[], payments:[], teachers:[],
    classes:[
      {id:1,name:'KG1',level:'Kindergarten',teacher:''},{id:2,name:'KG2',level:'Kindergarten',teacher:''},
      {id:3,name:'BS.1',level:'Basic School',teacher:''},{id:4,name:'BS.2',level:'Basic School',teacher:''},
      {id:5,name:'BS.3',level:'Basic School',teacher:''},{id:6,name:'BS.4',level:'Basic School',teacher:''},
      {id:7,name:'BS.5',level:'Basic School',teacher:''},{id:8,name:'BS.6',level:'Basic School',teacher:''},
    ...
    expenditures:[], resources:[], exams:[], transfers:[], announcements:[], parentNotifications:[],
    admissions:[], backupHistory:[], schoolLogo:null, driveClientId:'',
    settings:{
      schoolName:name, term:'First Term',
      session: new Date().getFullYear()+'/'+(new Date().getFullYear()+1),
      address:'', principal:adminName||'Administrator', district:'', motto:''
    },
    // Admin user is ACTIVE immediately — no approval step needed
  ...
  // 1. Save locally FIRST — works even offline, no Firebase dependency
  localStorage.setItem(schoolKey, JSON.stringify(freshData));

  // 2. Add to local registry so it appears in school selector immediately
  ...
  // 3. Sync to Firebase in the background (non-blocking — failure does NOT block registration)
  if (window._fbReady && _isOnline) {
    window._fb.set(fbSchoolPath(schoolId), freshData).catch(e => console.warn('[FB] school push:', e));
    window._fb.set('pending_schools/' + schoolId, {
    ...
    window._fb.set('school_credentials/' + schoolId, {
...
function saveToDB() {
  ...
  try {
    ...
    const data = {
      ...
      classSubjects: state.classSubjects||[],  // ✅ NEW: subject assignments per class
...
function loadSchoolData(schoolKey) {
  try {
    ...
    if (data.classSubjects)    state.classSubjects    = data.classSubjects;  // ✅ NEW
...
const state = {
  ...
  classSubjects: [],  // ✅ NEW: subject assignments per class [{classId, className, subjects:[]}]
  weeklyRecords: [],
  attendance: [],
  users: [
...
function autoFillReportFromPupil(pupilId) {
  ...
  // ✅ NEW: Auto-load subjects assigned to this pupil's class
  const assignedSubjects = getSubjectsForClass(p.cls);
  if (assignedSubjects.length && !_addedSubjects.length) {
    _addedSubjects = [...assignedSubjects];
    renderSubjectRows();
    showToast(`📚 Loaded ${assignedSubjects.length} subjects for ${p.cls}`);
  }
...
function autoCreateFeeRecord(pupil) {
  ...
  if (!existing) {
    // ✅ FIX: Auto-fill due amount from fee structure when available
    const structAmt = getFeeFromStructure(pupil.cls, term, academicYear) || 0;
    state.fees.push({
      id:          state.nextFeeId++,
      student:     name,
      cls:         pupil.cls,
      ...
      due:         structAmt,             // ✅ inherit class fee structure amount
      paid:        0,
      payments:    [],
      arrears:     0,
      ...auditCreate(),
...
function updateDashFeeBars() {
  ...
  if (!feeBarsEl) return;
  // Group fee records by class — use fee-structure-aware helpers
  const classFees = {};
  state.fees.forEach(f => {
    ...
    classFees[f.cls].due  += td;
    classFees[f.cls].paid += tp;
...
function renderStudents(filter='', cls='') {
  ...
  tbody.innerHTML = data.map((s,i)=>{
    ...
    // ✅ FIX: use fee-structure-aware helpers instead of raw feeRec.due/feeRec.paid
    const liveDue  = feeRec ? (getDueForRecord(feeRec) + (feeRec.arrears||0)) : 0;
    const livePaid = feeRec ? totalPaidForRecord(feeRec) : 0;
    const liveFeeStatus = feeRec ? getStatus(liveDue, livePaid) : (s.feeStatus || 'Unpaid');
    const feeNote = feeRec && liveDue === 0 ? `<small style="color:var(--yellow);display:block;font-size:10px;">⚠ Fee not set</small>` : '';
...
function initStudentPhotoUpload() {
  document.getElementById('sPhotoInput').addEventListener('change', function(e) {
    ...
    var prev = document.getElementById('sPhotoPreview');
...
function renderFeeStructureUI() {
  ...
  // Use ALL classes from state.classes (not just those with enrolled pupils)
  // This lets admins set fees before any pupils are added
  const classNames = state.classes && state.classes.length
    ? state.classes.map(c => c.name)
    : [...new Set(state.students.map(s => s.cls).filter(Boolean))].sort();

  if (!classNames.length) {
    wrap.innerHTML = '<p style="text-align:center;color:var(--text-muted);padding:20px;">No classes set up yet. Add classes in the Classes section first.</p>';
    return;
  }
...
function saveFeeStructureAndClose() {
  // Collect all visible fss-input fields and write them to state
  document.querySelectorAll('.fss-input').forEach(inp => {
    const cls    = inp.dataset.cls;
    const term   = inp.dataset.term;
    const year   = inp.dataset.year;
    const amount = parseFloat(inp.value) || 0;
    if (cls && term) setFeeStructure(cls, term, year, amount);
  });
  saveToDB();
  document.getElementById('feeStructureModal').classList.remove('open');
  renderFees();
  updateFeeStats();
...
// ═══════════════════════════════════════════════════════════
// CLASS SUBJECTS SYSTEM
// Assign subjects to classes so reports auto-load them
// ═══════════════════════════════════════════════════════════

/** Get subjects assigned to a class */
function getSubjectsForClass(className) {
  if (!state.classSubjects) return [];
  const entry = state.classSubjects.find(cs => cs.className === className);
  return entry ? (entry.subjects || []) : [];
}

/** Save subjects for a class */
function setSubjectsForClass(className, subjects) {
  if (!state.classSubjects) state.classSubjects = [];
  const idx = state.classSubjects.findIndex(cs => cs.className === className);
  if (idx >= 0) {
    state.classSubjects[idx].subjects = subjects;
  } else {
    state.classSubjects.push({ className, subjects });
  }
}

/** Render the class subjects assignment UI inside the Classes section */
function renderClassSubjectsUI() {
  const wrap = document.getElementById('classSubjectsWrap');
  if (!wrap) return;

  const GES_SUBJECTS = [
    'English Language','Mathematics','Science','Social Studies',
    'Religious & Moral Education','Ghanaian Language','French',
    'Information and Communication Technology','Creative Arts',
    'Physical Education','Career Technology','History','Computing'
  ];

  const classes = (state.classes || []).map(c => c.name);
  if (!classes.length) {
    wrap.innerHTML = '<p style="text-align:center;color:var(--text-muted);padding:16px;">No classes yet. Add classes first.</p>';
    return;
  }

  let html = '<div style="display:grid;gap:14px;">';
  classes.forEach(cls => {
    const assigned = getSubjectsForClass(cls);
    const safeId   = cls.replace(/[^a-z0-9]/gi,'_');
    const pills    = assigned.length
      ? assigned.map(s => `<span style="display:inline-flex;align-items:center;gap:4px;background:var(--blue-light);color:var(--blue);border-radius:20px;padding:3px 10px;font-size:12px;font-weight:600;margin:2px;">
          ${escHtml(s)}
          <button type="button" onclick="removeClassSubject('${escHtml(cls).replace(/'/g,"\\'")}','${escHtml(s).replace(/'/g,"\\'")}'); renderClassSubjectsUI();" style="background:none;border:none;cursor:pointer;color:var(--blue);font-size:12px;padding:0;line-height:1;" title="Remove"><i class="fas fa-times"></i></button>
        </span>`).join('')
      : '<span style="font-size:12px;color:var(--text-muted);">No subjects assigned yet.</span>';

    html += `<div style="border:1px solid var(--border);border-radius:var(--radius-sm);padding:12px 14px;background:var(--surface);">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;flex-wrap:wrap;gap:8px;">
        <strong style="font-size:13px;">${escHtml(cls)}</strong>
        <div style="display:flex;gap:6px;align-items:center;">
          <select class="form-input cs-picker" id="csPicker_${safeId}" style="font-size:12px;padding:5px 8px;min-width:180px;">
            <option value="">-- Add Subject --</option>
            ${GES_SUBJECTS.filter(s=>!assigned.includes(s)).map(s=>`<option value="${escHtml(s)}">${escHtml(s)}</option>`).join('')}
          </select>
          <button class="btn-primary" style="font-size:12px;padding:5px 12px;" onclick="addClassSubjectFromPicker('${escHtml(cls).replace(/'/g,"\\'")}','csPicker_${safeId}')">
...
function addClassSubjectFromPicker(className, pickerId) {
  const picker = document.getElementById(pickerId);
  if (!picker || !picker.value) { showToast('⚠️ Select a subject first.'); return; }
  const sub = picker.value;
  const current = getSubjectsForClass(className);
  if (current.includes(sub)) { showToast('⚠️ Already assigned.'); return; }
  setSubjectsForClass(className, [...current, sub]);
  autosave();
  renderClassSubjectsUI();
  showToast(`✅ "${sub}" added to ${className}`);
}

function removeClassSubject(className, subject) {
  const current = getSubjectsForClass(className);
  setSubjectsForClass(className, current.filter(s => s !== subject));
  autosave();
}
// ─────────────────────────────────────────────────────────
...
function printStudentStatement(studentName, studentId) {
  ...
  const termSections = allRecs.map(rec => {
    const payments = rec.payments && rec.payments.length
      ? rec.payments
      : (rec.paid > 0 ? [{ amt: rec.paid, date: rec.createdAt?.slice(0,10)||'-', note:'', method:'Cash', receiptNo:'-' }] : []);
    const termDue  = getDueForRecord(rec) + (rec.arrears||0);  // ✅ FIX: use fee-structure-aware helper
    const termPaid = payments.reduce((a,p) => a + p.amt, 0);
    const termBal  = termDue - termPaid;
    grandDue  += termDue;
    grandPaid += termPaid;
...
function renderArrearsTable() {
  ...
  // ✅ FIX: use fee-structure-aware helpers for accurate balance calculation
  ...
  data = data.map(f => ({
    ...f,
    _liveDue:     getDueForRecord(f) + (f.arrears||0),
    _livePaid:    totalPaidForRecord(f),
  ...
  wrap.innerHTML = `
    ...
    <div class="table-wrap"><table class="data-table" id="arrearsTable">
      ...
      <tbody>${data.map((f,i) => `<tr>
        ...
        <td>${fmt(f._liveDue)}</td>
        <td style="color:var(--green);">${fmt(f._livePaid)}</td>
...
function printArrearsReport() {
  ...
  // ✅ FIX: use fee-structure-aware helpers
  let data = state.fees.filter(f => {
    const td = getDueForRecord(f) + (f.arrears||0);
    return td > 0 && td > totalPaidForRecord(f);
  });
  if (clsF && clsF !== 'All Classes') data = data.filter(f => f.cls === clsF);
  data = data.map(f => {
    const td = getDueForRecord(f) + (f.arrears||0);
    const tp = totalPaidForRecord(f);
    return {...f, _liveDue: td, _livePaid: tp, balance: td - tp};
  }).sort((a,b)=>b.balance-a.balance);
  const total = data.reduce((a,f)=>a+f.balance,0);
  const rows = data.map((f,i)=>`<tr><td>${i+1}</td><td>${f.student}</td><td>${f.cls||'-'}</td><td>${f.term||'-'}</td><td>${fmt(f._liveDue)}</td><td>${fmt(f._livePaid)}</td><td style="color:#dc2626;font-weight:700;">${fmt(f.balance)}</td></tr>`).join('');
...
function initGallery() {
  ...
  document.getElementById('photoUploadInput').addEventListener('change', function(e) {
    var album = state.albums.find(function(a){ return a.id === currentAlbumId; });
    if (!album) return;
...
function renderClasses() {
  ...
  if (!state.classes.length){ tbody.innerHTML=`<tr><td colspan="6" style="text-align:center;padding:28px;color:var(--text-light);">No classes added.</td></tr>`; grid.innerHTML=''; renderClassSubjectsUI(); return; }
  ...
  // ✅ NEW: Refresh subject assignment panel
  renderClassSubjectsUI();
}
...
function initClasses() {
  renderClasses();
  renderClassSubjectsUI(); // ✅ NEW: Initialize subject assignment panel
...
function saveClass() {
  ...
  if (editId){
    ...
    // ✅ FIX: Prevent duplicate class names
    if (state.classes.some(c => c.name.toLowerCase() === name.toLowerCase())) {
      showToast(`⚠️ A class named "${name}" already exists.`); return;
    }
  ...
  submitSchoolRegistration(name, adminName, adminUser, adminPass, phone, email)
    .then(() => {
      ...
      ['newSchoolName','newSchoolAdmin','newSchoolUsername','newSchoolPassword','newSchoolPhone','newSchoolEmail']
        .forEach(id => { const el = document.getElementById(id); if(el) el.value = ''; });
      // Refresh school list so the new school appears immediately
      renderSchoolList();
      // Show success screen
    ...
    .finally(() => {
      btn.disabled = false;
      btn.innerHTML = '<i class="fas fa-paper-plane"></i> Register School';
    });
}
...
function initLogin() {
  ...
  // Register new school - open modal
  const registerNewSchoolBtn = document.getElementById('registerNewSchoolBtn');
  if (registerNewSchoolBtn) registerNewSchoolBtn.addEventListener('click', () => {
    document.getElementById('registerFormError').style.display = 'none';
    ['newSchoolName','newSchoolAdmin','newSchoolUsername','newSchoolPassword','newSchoolPhone'].forEach(id=>{const el=document.getElementById(id);if(el)el.value='';});
    document.getElementById('registerSchoolModal').classList.add('open');
  });

  // Register school modal close/cancel
  const closeRegModal = document.getElementById('closeRegisterModal');
  if (closeRegModal) closeRegModal.addEventListener('click', () => document.getElementById('registerSchoolModal').classList.remove('open'));
  const cancelRegModal = document.getElementById('cancelRegisterModal');
  if (cancelRegModal) cancelRegModal.addEventListener('click', () => document.getElementById('registerSchoolModal').classList.remove('open'));
  const confirmRegBtn = document.getElementById('confirmRegisterBtn');
  if (confirmRegBtn) confirmRegBtn.addEventListener('click', registerNewSchool);
...
function viewPortalReport(reportId) {
  var r = (state.reports||[]).find(function(rep){ return rep.id === reportId; });
  if (!r) { showToast("Report not found."); return; }
  var s = state.settings;
  var win = window.open("","_blank");
  if (!win) { showToast("Please allow popups to view the report."); return; }
  var rows = (r.subjects||[]).map(function(sub) {
    var total = sub.total || 0;
    var color = total >= 80 ? '#16a34a' : total >= 60 ? '#0891b2' : total >= 50 ? '#d97706' : '#dc2626';
    return "<tr><td style='padding:8px 12px;border-bottom:1px solid #f0f0f0;'>"+sub.name+"</td>"
      +"<td style='padding:8px 12px;border-bottom:1px solid #f0f0f0;text-align:center;'>"+(sub.classScore||"-")+"</td>"
      +"<td style='padding:8px 12px;border-bottom:1px solid #f0f0f0;text-align:center;'>"+(sub.examScore||"-")+"</td>"
      +"<td style='padding:8px 12px;border-bottom:1px solid #f0f0f0;text-align:center;font-weight:800;color:"+color+";'>"+(sub.total||"-")+"</td>"
      +"<td style='padding:8px 12px;border-bottom:1px solid #f0f0f0;text-align:center;font-weight:700;'>"+(sub.grade||"-")+"</td></tr>";
  }).join("");
  var logoHtml = state.schoolLogo ? '<img src="'+state.schoolLogo+'" style="height:60px;object-fit:contain;"/>' : '<span style="font-size:36px;">🏫</span>';
  win.document.write("<!DOCTYPE html><html><head><title>Report Card - "+(r.studentName||"")+"</title>"
    +"<style>*{box-sizing:border-box;margin:0;padding:0;} body{font-family:'Segoe UI',Arial,sans-serif;padding:24px;max-width:760px;margin:auto;color:#1a2133;}"
    +"h1{color:#1a6fd4;font-size:20px;text-align:center;margin:8px 0 4px;} h2{font-size:14px;color:#64748b;text-align:center;margin-bottom:4px;}"
    +".badge{display:inline-block;background:#1a6fd4;color:#fff;padding:4px 14px;border-radius:20px;font-size:12px;font-weight:700;margin:6px auto 0;}"
    +".info-grid{display:grid;grid-template-columns:1fr 1fr;gap:6px;background:#f4f7fc;border-radius:8px;padding:12px 16px;margin:14px 0;font-size:13px;}"
    +".info-item{display:flex;flex-direction:column;} .lbl{font-size:10px;color:#94a3b8;font-weight:700;text-transform:uppercase;} .val{font-weight:700;color:#1a2133;margin-top:1px;}"
    +"table{width:100%;border-collapse:collapse;font-size:13px;border:1px solid #dde3ef;border-radius:8px;overflow:hidden;}"
    +"th{background:#1a6fd4;color:#fff;padding:10px 12px;text-align:left;font-size:12px;font-weight:700;}"
    +"th:not(:first-child){text-align:center;}"
    +".remarks{background:#f0fdf4;border:1px solid #86efac;border-radius:8px;padding:12px 16px;margin:14px 0;font-size:13px;}"
    +".footer{text-align:center;font-size:11px;color:#94a3b8;margin-top:20px;padding-top:12px;border-top:1px solid #f0f0f0;}"
    +"@media print{button{display:none;}}</style></head><body>"
    +"<div style='text-align:center;margin-bottom:12px;'>"+logoHtml+"</div>"
    +"<h1>"+(s.schoolName||"School")+"</h1>"
    +"<h2>Ghana Education Service"+( s.district ? ' · '+s.district : '')+"</h2>"
    +"<div style='text-align:center;'><span class='badge'>"+( r.term||"Term")+" REPORT CARD "+( r.year||"")+"</span></div>"
    +"<div class='info-grid'>"
    +"<div class='info-item'><span class='lbl'>Pupil Name</span><span class='val'>"+(r.studentName||"—")+"</span></div>"
    +"<div class='info-item'><span class='lbl'>Class</span><span class='val'>"+(r.cls||"—")+"</span></div>"
    +"<div class='info-item'><span class='lbl'>Position</span><span class='val'>"+(r.position ? r.position+" / "+(r.classSize||"—") : "—")+"</span></div>"
    +"<div class='info-item'><span class='lbl'>Attendance</span><span class='val'>"+(r.daysPresent ? r.daysPresent+" / "+(r.totalDays||"—")+" days" : "—")+"</span></div>"
    +"</div>"
    +"<table><thead><tr><th>Subject</th><th>Class Score</th><th>Exam Score</th><th>Total</th><th>Grade</th></tr></thead>"
    +"<tbody>"+rows+"</tbody></table>"
    +(r.remarks ? "<div class='remarks'><strong>Teacher's Remarks:</strong> "+r.remarks+"</div>" : "")
    +(r.hmRemark ? "<div class='remarks' style='background:#e8f0fb;border-color:#bfdbfe;'><strong>Head Teacher's Remark:</strong> "+r.hmRemark+"</div>" : "")
    +"<div class='footer'>EduManage Pro · "+(s.schoolName||"School")+" · Generated "+new Date().toLocaleString('en-GH')+"</div>"
    +"<div style='text-align:center;margin-top:16px;'><button onclick='window.print()' style='padding:8px 24px;background:#1a6fd4;color:#fff;border:none;border-radius:6px;cursor:pointer;font-weight:700;font-size:14px;'>🖨️ Print Report Card</button></div>"
    +"</body></html>");
  win.document.close();
}
...
function printPortalReport(reportId) {
  ...
  var s = state.settings;
  var win = window.open("","_blank");
  if (!win) { showToast("Please allow popups to print."); return; }
  var rows = (r.subjects||[]).map(function(sub) {
    return "<tr><td>"+sub.name+"</td><td>"+(sub.classScore||"-")+"</td><td>"+(sub.examScore||"-")+"</td><td>"+(sub.total||"-")+"</td><td>"+(sub.grade||"-")+"</td></tr>";
  }).join("");
  win.document.write("<!DOCTYPE html><html><head><title>Report Card</title>"
    ...
    +"th{background:#1a6fd4;color:#fff;}</style></head><body>"
    +"<h2 style='color:#1a6fd4;text-align:center'>"+(s.schoolName||"School")+"</h2>"
    +"<p><strong>Name:</strong> "+(r.studentName||"")+" | <strong>Class:</strong> "+(r.cls||"")+" | <strong>Term:</strong> "+(r.term||"")+" "+(r.year||"")+"</p>"
    +"<table><tr><th>Subject</th><th>Class Score</th><th>Exam Score</th><th>Total</th><th>Grade</th></tr>"
    +rows+"</table>"
